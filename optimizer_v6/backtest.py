"""
backtest.py — Moteur de backtest et métriques optimizer_v6
"""
import numpy as np
...
def backtest_single(df, pr, sp, strat):
    """Backtest un seul actif avec position fixe. Retourne trades list."""
    trades=[]; opens=[]; lb=pr.get("range_lb",5); spread=sp/100.0
    for i in range(max(lb+30, 50), len(df)):
        row=df.iloc[i]; new_o=[]
        for ot in opens:
            ...
            if not closed:
                ...
                if sl_hit:
                    ...
                    closed=True
            if not closed and held>=pr.get("max_hold",5):
...
def calc_m(trades, risk_eur=110):
    n=len(trades)
