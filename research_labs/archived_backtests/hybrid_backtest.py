...
║                     A P E X     E N G I N E                                      ║
║                                                                                  ║
║    Division 1  "THE FLEET"    — 1H Day Trading × 20 assets                       ║
║    Division 2  "THE SNIPERS"  — 5m Scalping   × BTCUSD + GOLD                   ║
...
def fleet_signal(df, i):
    """High-Velocity 1H signal — proven breakout + MR + trend."""
    if i < 30:
        return "HOLD", ""
    row  = df.iloc[i]
    prev = df.iloc[i - 1]
    c = row["close"]; rsi = row["rsi"]; atr = row["atr"]
    ...
    # ── BREAKOUT ── range break + EMA alignment + volume
    ...
    bb_d = row.get("bb_d", c); bb_u = row.get("bb_u", c)
    if rsi < FLEET_RSI_LO and c <= bb_d * 1.002 and c > prev["close"]:
        return "BUY", "MR"
    if rsi > FLEET_RSI_HI and c >= bb_u * 0.998 and c < prev["close"]:
        return "SELL", "MR"

    # ── TREND FOLLOW ── MACD zero-line cross + strong EMA trend
    if ef > es and mh > 0 and prev["macd_h"] <= 0 and vr > 1.0:
        return "BUY", "TF"
    if ef < es and mh < 0 and prev["macd_h"] >= 0 and vr > 1.0:
        return "SELL", "TF"
